...
# Importar librerías
import pandas as pd     # Para manipulación de datos en forma de DataFrames
import numpy as np      # Para operaciones numéricas
import matplotlib.pyplot as plt    # Para visualización de gráficos
import seaborn as sns     # Para visualización de gráficos avanzada
import collections   # Para manipulación de colecciones de datos
import tensorflow as tf      # Para implementar modelos de aprendizaje profundo

# Importar módulos específicos de Keras
...
from sklearn.linear_model import LogisticRegression      # Para modelos de regresión logística
from sklearn.metrics import precision_recall_curve, roc_curve     # Para curvas de precisión y recuperación, curvas ROC

"""###Datasets"""

X = pd.read_csv("/content/drive/MyDrive/dataset/ptbdb_normal.csv", header=None)    # Cargar el archivo CSV "ptbdb_normal.csv" en un DataFrame llamado X
Y = pd.read_csv("/content/drive/MyDrive/dataset/ptbdb_abnormal.csv", header=None)   # Cargar el archivo CSV "ptbdb_abnormal.csv" en un DataFrame llamado Y
df = pd.concat([X,Y])     # Concatenar los DataFrames X e Y en un nuevo DataFrame llamado df
...
df.head(5)  # Mostrar las primeras 5 filas del DataFrame df
...
df_train, df_test = train_test_split(df, test_size=0.2, random_state=1337, stratify=df[187])   # Dividir los datos en conjuntos de entrenamiento 
...
n_epochs = 10    #Definimos el número de epocas que se entrenará el modelo
...
# Obtener un modelo de línea de base utilizando una función llamada get_baseline_model()
model = get_baseline_model()

# Imprimir un resumen del modelo que muestra la arquitectura de la red neuronal
model.summary()

# Generar un diagrama del modelo y guardarlo como 'model.png'
plot_model(model, show_shapes=True, show_layer_names=True, to_file='model.png')

# Compilar el modelo con el optimizador Adam, una tasa de aprendizaje de 0.001,
# la función de pérdida binary_crossentropy y métricas de precisión ('accuracy')
model.compile(optimizer=optimizers.Adam(lr=0.001), loss=losses.binary_crossentropy, metrics=['accuracy'])

# Entrenar el modelo utilizando los conjuntos de características (X) y etiquetas (Y)
# durante un número determinado de épocas (n_epochs) y un tamaño de lote de 16.
# También se utilizan callbacks (checkpoint, early_stop, reduce_lr) para controlar el entrenamiento
# y se realiza una validación utilizando el 10% de los datos como conjunto de validación.
history = model.fit(X, Y, epochs=n_epochs, batch_size=16, callbacks=[checkpoint, early_stop, reduce_lr], validation_split=0.1)

"""###Evaluación del modelo"""

# Cargar los pesos entrenados en el modelo desde un archivo
...
# Calcular la precisión, el área bajo la curva ROC y el valor F1 utilizando las etiquetas reales (Y_test) y las predicciones (pred_test)
